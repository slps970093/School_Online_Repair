        ...
        <div class="main">
          ...
          <?php echo form_open('admin/device/repair/search',array('method' => 'GET')); ?>
            <table class="table">
              <tr>
                <td>搜尋教室：</td>
                <td><input type="text" name="location" class="form-control" value=""></td>
                <td>選擇學年度：</td>
                <td><select class="form-control" name="year_id">
                    <option>不限</option>
                    <?php foreach ($year_result as $row) { ?>
                    <?php echo "<option value='".$row['id']."'>".$row['yi_name']."</option>"; ?>
                    <?php } ?>
                    </select></td>
                <td>選擇狀態：</td>
